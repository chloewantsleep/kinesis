...
// ===== Vibration Motors =====
#define MOTOR_LEFT_SHOULDER 18
#define MOTOR_RIGHT_SHOULDER 19
#define MOTOR_UPPER_SPINE 23
#define MOTOR_LOWER_SPINE 5 // D5
...
MotorState motors[NUM_MOTORS] = {
    {MOTOR_LEFT_SHOULDER, "left_shoulder", false, 0},
    {MOTOR_RIGHT_SHOULDER, "right_shoulder", false, 0},
    {MOTOR_UPPER_SPINE, "upper_spine", false, 0},
    {MOTOR_LOWER_SPINE, "lower_spine", false, 0},
};
             ...
             float &temp)
{
  ...
  if (Wire.requestFrom(addr, 14) < 14)
    return false;
...
void sendAllIMUFrames(unsigned long ts)
{
  for (int i = 0; i < NUM_IMUS; i++)
  {
    ...
    if (!mpuRead(IMU_ADDRS[i], ax, ay, az, gx, gy, gz, temp))
      continue;
...
void handleSerialCommands()
{
  ...
  if (strcmp(cmd, "vibrate") == 0)
  {
    ...
    int pwm = doc["pwm"] | 180;
    const char *motor = doc["motor"]; // optional: "left_shoulder" | "right_shoulder" | "upper_spine" | "lower_spine"
    ...
    {
      // No motor specified → fire all four
      for (int i = 0; i < NUM_MOTORS; i++)
        activateMotor(i, pwm, duration_ms, now);
      ack["motor"] = "all";
    }
